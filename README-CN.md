...
| `/rewind`                | 列出快照点，预览差异并还原文件（撤销之后的改动）                                                         |
| `/snapshots [--cleanup]` | 查看快照占用与保留策略（`--cleanup` 触发过期清理）                                                       |
| `/plan [请求]`           | 计划模式：只读研究 → 结构化计划 → 批准/修改/拒绝（批准后切执行模式）；`load <路径>` 从 markdown 读回计划 |
...
技能是「怎么做某类事」的分步流程知识，与项目指令互补：`AGENTS.md` 是**总是适用**的事实，技能只对**特定任务**适用（代码审查、写测试、调试排查…）。modou 遵循 [Agent Skills](https://agentskills.io) 开放标准——把一个未改造的第三方技能目录（内含 `SKILL.md`）放进任一层级即可生效。

技能按三层发现（后一层覆盖前一层同名技能）：

1. 随包发布的内置 `skills/`——最低优先级
2. 全局 `~/.modou/skills/`
