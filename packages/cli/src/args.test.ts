...
describe('parseArgs（-p 参数解析）', () => {
  test('-p 与 --prompt 等价，支持 --prompt= 形式', () => {
    expect(parseArgs(['-p', '你好'])).toEqual({ prompt: '你好', help: false });
  ...
  test('--help 返回帮助标志，可与 -p 共存', () => {
    expect(parseArgs(['--help'])).toEqual({ prompt: undefined, help: true });
    expect(parseArgs(['-h'])).toEqual({ prompt: undefined, help: true });
  ...
  test('空参数：未给 -p 且无 --help → prompt 为 undefined', () => {
    expect(parseArgs([])).toEqual({ prompt: undefined, help: false });
  });
